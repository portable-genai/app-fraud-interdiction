...
from app_fraud_interdiction.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "SCAMINTERDICT_AUDIT_ANCHOR"
...
_EVENTS = (sample_cases.ESCALATING_EVENT, sample_cases.ROUTINE_EVENT, sample_cases.PII_EVENT)
...
def _record_three(settings: Settings) -> Container:
    ...
    assert isinstance(container.audit, LocalAuditAdapter)
    service = build_service(container)
    for event in _EVENTS:
        service.assess(event, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT)
    return container
