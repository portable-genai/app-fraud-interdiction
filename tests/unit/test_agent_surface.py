...
from app_fraud_interdiction.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    assess_payment,
...
def _assess_block(**overrides: object) -> dict[str, object]:
    kwargs: dict[str, object] = {
        "event_id": "sg-block-agent",
        "market": "SG",
    ...
    return assess_payment(**kwargs)  # type: ignore[arg-type]
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "app-fraud-interdiction"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    result = _assess_block(actor=sample_cases.ACTOR, tenant=sample_cases.TENANT)
...
def test_the_stream_listing_tool_returns_pending_payments() -> None:
    listed = list_payment_stream(market="SG", settings=local_settings())
    assert listed["events"], "the stream listing must return the pending payments"
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    data. A planted identifier in the caller-supplied event id must be masked on the way out.
    """
    result = _assess_block(event_id=f"case {sample_cases.PLANTED_NRIC}")
