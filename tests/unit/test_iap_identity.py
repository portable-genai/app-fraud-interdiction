...
from app_fraud_interdiction.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from app_fraud_interdiction.config import Settings, _read_settings_file
from app_fraud_interdiction.ports.identity import (
    VERIFIED,
...
_API_MODULE = "app_fraud_interdiction.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The domain-pipeline ports are rebound to their LOCAL adapters under the managed profile
        # too, so the verified-assertion path runs end to end offline while identity stays gcp.
        "  payment_stream:",
        *[
            f"    {p}: {_PKG}.adapters.local.payment_stream:LocalPaymentStream"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.payment_stream:OnPremPaymentStream",
        "  features:",
        *[f"    {p}: {_PKG}.adapters.local.features:LocalFeatureStore" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.features:OnPremFeatureStore",
        ...
        *[
        ...
        "  warning_generator:",
        *[
            f"    {p}: {_PKG}.adapters.local.warning_generator:LocalWarningGenerator"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.warning_generator:OnPremWarningGenerator",
    ]
)
...
def test_an_emptied_audience_variable_refuses_at_load_rather_than_resolving_to_empty(
    ...
    ``iap_audience: ${SCAMINTERDICT_IAP_AUDIENCE:-}`` goes through the same
    ``setting_or_default`` the adapters use, so an operator who emptied the variable gets a boot
    failure naming it rather than a service that silently verifies against no audience.
    """
    path = tmp_path / "settings.yaml"
    path.write_text(_REBOUND_SETTINGS + "\n", encoding="utf-8")
