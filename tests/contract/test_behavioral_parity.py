...
from app_fraud_interdiction.adapters._review_payload import (
    result_to_review,
)
...
def test_the_audit_record_is_redacted_before_anything_leaves_the_process() -> None:
    """The memo carries planted PII; the redact-before-audit rule must mask it in the WORM record.

    The personal data enters through the payment memo and is masked before the audit write, so no
    raw identifier reaches the hash-chained record. This is the redact-before-anything path for
    the interdiction vertical, where the review payload is built from the engine's own cited
    output rather than from customer free text.
    """
    container = build_container(local_settings())
    service = build_service(container)
    service.assess(sample_cases.PII_EVENT, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT)
    records = [str(entry.get("redacted_summary", "")) for entry in container.audit.log.read_all()]
    joined = " ".join(records)
    assert records, "the assessment must have written an audit record"
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_service(local).assess(
