# Scam and APP Interdiction (G3)

Real-time deterministic scam and APP-payment interdiction with cited warnings and human review.
...
One env var, `SCAMINTERDICT_PROFILE`, selects the adapter family:
...
`SCAMINTERDICT_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`SCAMINTERDICT_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the interdiction service redacts with |
...
**Name the profile.** `SCAMINTERDICT_PROFILE` has no default. Leaving it unset is
