"""FastAPI application for Scam and APP Interdiction (G3).
...
``SCAMINTERDICT_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``SCAMINTERDICT_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real interdiction decision.
...
from .schemas import HealthResponse, InterdictRequest, InterdictResponse
...
_INSECURE_DEMO_ENV = "SCAMINTERDICT_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "SCAMINTERDICT_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "SCAMINTERDICT_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "SCAMINTERDICT_S2S_AUDIENCE"
_API_HOST_ENV = "SCAMINTERDICT_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``SCAMINTERDICT_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``SCAMINTERDICT_PROFILE`` must never be allowed to
    ...
    ``SCAMINTERDICT_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "SCAMINTERDICT_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "app-fraud-interdiction"
...
_APP_TITLE = "Scam and APP Interdiction"
...
# pill names is what answered, never what configuration says would. A request whose warning
# generator was never reached, or whose managed draft failed, noted nothing and sends neither;
# the pill then keeps showing the configured `generator_model` from `/healthz`.
...
#
# Note what is NOT in this expression: SCAMINTERDICT_S2S_TOKEN. A service
...
@app.post("/v1/interdict", response_model=InterdictResponse, tags=["artifacts"])
def interdict(
    request: InterdictRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> InterdictResponse:
    """Assess one in-flight payment; the audit actor is the verified principal, never the body.
    ...
    result = service.assess(request.to_event(), actor=principal.actor, tenant=principal.tenant)
    ...
    return InterdictResponse.from_domain(
