...
One env var (``SCAMINTERDICT_PROFILE``) selects the adapter family for every
...
from .ports.payment_stream import PaymentStreamPort
from .ports.review_router import ReviewRouterPort
from .ports.warning_generator import WarningGeneratorPort

_PROFILE_ENV = "SCAMINTERDICT_PROFILE"
_SETTINGS_ENV = "SCAMINTERDICT_SETTINGS"
...
_GENERATOR_PORT: str = "warning_generator"
...
class ProfileChoice:
    ...
    The variable is ``SCAMINTERDICT_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("SCAMINTERDICT_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "payment_stream": {
        "local": f"{_PKG}.adapters.local.payment_stream:LocalPaymentStream",
        "gcp": f"{_PKG}.adapters.gcp.payment_stream:BigQueryPaymentStream",
        "onprem": f"{_PKG}.adapters.onprem.payment_stream:OnPremPaymentStream",
    },
    "features": {
        "local": f"{_PKG}.adapters.local.features:LocalFeatureStore",
        "gcp": f"{_PKG}.adapters.gcp.features:VertexFeatureStore",
        "onprem": f"{_PKG}.adapters.onprem.features:OnPremFeatureStore",
    },
    "conversation_channel": {
        "local": f"{_PKG}.adapters.local.conversation_channel:LocalConversationChannel",
        "gcp": f"{_PKG}.adapters.gcp.conversation_channel:CcaiConversationChannel",
        "onprem": f"{_PKG}.adapters.onprem.conversation_channel:OnPremConversationChannel",
    },
    "warning_generator": {
        "local": f"{_PKG}.adapters.local.warning_generator:LocalWarningGenerator",
        "gcp": f"{_PKG}.adapters.gcp.warning_generator:VertexWarningGenerator",
        "onprem": f"{_PKG}.adapters.onprem.warning_generator:OnPremWarningGenerator",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``SCAMINTERDICT_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "SCAMINTERDICT_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def payment_stream(self) -> PaymentStreamPort:
        adapter = self._bind("payment_stream")
        assert isinstance(adapter, PaymentStreamPort)
        return adapter

    @cached_property
    def features(self) -> FeaturePort:
        adapter = self._bind("features")
        assert isinstance(adapter, FeaturePort)
        return adapter
    ...
    @cached_property
    def warning_generator(self) -> WarningGeneratorPort:
        adapter = self._bind("warning_generator")
        assert isinstance(adapter, WarningGeneratorPort)
        return adapter
