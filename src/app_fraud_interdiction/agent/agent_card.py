...
class AgentCard:
    ...
    provider: str = "app-fraud-interdiction"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="assess_payment",
        name="Payment interdiction",
        description=(
            "Score one in-flight payment into a deterministic allow / warn / hold / block "
            "verdict, record an already-redacted audit event, and ROUTE a hold or block to human "
            "review (rule R8). The verdict is computed by pure stdlib code, never by a model."
        ),
    ),
    AgentSkill(
        id="list_payment_stream",
        name="Payment stream listing",
        description=(
            "List the in-flight payments waiting on the stream, filtered by market, without "
            "scoring them. Personal data is masked before the listing reaches a model."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://app-fraud-interdiction.{settings.region}.internal.example/a2a"
