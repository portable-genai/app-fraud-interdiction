...
``SCAMINTERDICT`` environment prefix behind every ``SCAMINTERDICT_PROFILE``-style variable, the
distribution and resource id ``app-fraud-interdiction`` and the Terraform
...
``app_fraud_interdiction``, so ``--package`` renames the CLI as well and a
...
_OLD_PACKAGE = "app_fraud_interdiction"
_OLD_ENV_PREFIX = "SCAMINTERDICT"
_OLD_RESOURCE = "app-fraud-interdiction"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        if old == _OLD_ENV_PREFIX:
            # Only where it IS the environment prefix: ``SCAMINTERDICT_NAME``, the
            # ``SCAMINTERDICT_*`` glob a comment writes, and the bare token in
...
def main() -> int:
    ...
    ap.add_argument("--env-prefix", required=True, help="new env-var prefix, e.g. ACMESCAM")
    ap.add_argument(
        "--resource",
        required=True,
        help="new distribution / resource id, kebab-case, e.g. acme-scam-interdiction",
    )
    ap.add_argument(
        "--name-prefix",
        default="",
        help="new Terraform name_prefix default, e.g. acme-scam (default: leave it alone)",
    )
    ...
    if not re.fullmatch(r"[A-Z][A-Z0-9_]*", args.env_prefix):
        ap.error("--env-prefix must be an UPPER_SNAKE token, e.g. ACMESCAM")
    if not re.fullmatch(r"[a-z0-9]+(-[a-z0-9]+)*", args.resource):
        ap.error("--resource must be a kebab-case id, e.g. acme-scam-interdiction")
    if args.name_prefix and not re.fullmatch(r"[a-z][a-z0-9-]{2,18}", args.name_prefix):
        ...
        ap.error("--name-prefix must match ^[a-z][a-z0-9-]{2,18}$, e.g. acme-scam")
