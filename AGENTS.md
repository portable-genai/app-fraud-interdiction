...
Scam and APP Interdiction (G3): Real-time deterministic scam and APP-payment interdiction with cited warnings and human review.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `app_fraud_interdiction`, environment prefix
`SCAMINTERDICT`, region `asia-southeast1`.
...
- `src/app_fraud_interdiction/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `SCAMINTERDICT_S2S_TOKEN` may never enter that decision: it authenticates a
  ...
  persona and a real interdiction decision. `tests/unit/test_end_user_auth_posture.py` walks the
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `SCAMINTERDICT_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `SCAMINTERDICT_PROFILE` (mentioning it in a refusal message is wanted, reading
